...
RUN apk del --no-cache .build-deps \
    ...
    && echo "JAVA_HOME=/usr/lib/jvm/java-11-openjdk" | tee -a /etc/profile \
...
RUN echo "JAVA_HOME=/usr/lib/jvm/java-11-openjdk" | tee -a /etc/profile \
    && echo "export JAVA_HOME=/usr/lib/jvm/java-11-openjdk" | tee -a /etc/profile \
...
RUN if [ "$MODE" = "dev" ]; then \
    ...
    && pecl install -o -f xdebug-$XDEBUG_PECL_VERSION \
...
COPY --from=opensearchproject/opensearch:2.19.1 --chown=opensearch:opensearch /usr/share/opensearch /usr/share/opensearch
